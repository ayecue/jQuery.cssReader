...
var cssReader = function (options){
  var generic = {
	...
	cssReader.extend(this,{
		...
		filterCss:function (filterArray)
		{
			var filterString=typeof filterArray != "string" ? filterString=filterArray.join("|") : filterArray;
			
			if (!(filterString in this.patternFilter))
				this.patternFilter[filterString]=new RegExp("[^{}]+{[^{}]*(?:"+filterString+")[^{}]*}","gi");
				
			this.d.plainCss=this.d.plainCss.match(this.patternFilter[filterString]).join("");
		},
		get:function (filter)
		{
			var self=this;
			if (this.d.targetUrl)
			{
				return $.ajax({
							...
							success: function (css) {self.setCss(css,filter ? filter : null);}
				});
			}
		},
		...
		scrape:function(callback,filterArray)
		{
			var self=this;
			$.when(filterArray ? self.get(filterArray) : self.get()).done(function(){
